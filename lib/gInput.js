...
var gInput = {
  'objectDifference':function(collection,obj){
    ...
    var nullInput = []
    if(popObj){
      var idx = _.findIndex(data,popObj);

      if(idx>=0){
        data.splice(idx,1)
      }
      this.objectDifference(data,obj)
    }else{
      return data;
    }
  },
  ...
  'generateNullTestCase':function (data){
    ...
    var normal = sort.pop()
    console.log(_this.objectDifference([1,2,3,4],[3]))
    console.log('ok')
    _.forEach(sort,function(n,key){
      ...
      _.forEach(_nullInput,function(_n,key){
        /*if(_.indexOf(n.validation,'required')<=0){
          input.result.push(messages.messages.required)
        }*/
      })
