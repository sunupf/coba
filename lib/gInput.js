...
var gInput = {
  ...
  'registerCustomValidatorJS' : function(){
    try {
      var customValidators = require(process.cwd()+'/validations')
      _.forEach(customValidators,function(n,key){
  ...
  'getFlatRules' : function(configs){
    ...
    _.forEach(configs,function(config,index){
      flatRules[config.name] = config.validation.join("|")
    })
    return flatRules;
  },
  'getNormalizeRules' : function(configs,inputs){
    var normalRules = {}
    _.forEach(inputs,function(input,index){
      var idx = _.findIndex(configs, function(config) {
        return config.selector === index;
      })
      normalRules[configs[idx].name] = input
    })
    return normalRules;
  },
  'generateOutputResults' : function (inputs,configs){
    ...
    _.forEach(inputs,function(input,key){
      ...
      var data = _this.getNormalizeRules(configs,input.testCases);
      var validation = new Validator(data,rules,messages.messages);
      if(validation.passes()){
        inputs[key].messages = [messages.messages.success];
      }else{
        var errors = []
        _.forEach(validation.errors.errors,function(error,index){
          errors.push(error)
        })
        var flattenErrors = _.flatten(errors);
        inputs[key].messages = flattenErrors;
      }
    })
