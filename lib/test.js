// TODO
// get config
// loop based on browser that specified
// read provided data
// loop n(data) times
  // open Url
  // record time when loaded
  // insert specified input
  // do custom scenario that specified by user in config.scenarion
  // submit action
  // record time when success/fail
// save the record to json file
// show the result in pretier format

var Test = function(config){
  this.log = [];
  this.result = "";
  this.config = config;
  this.startTime = 0;
  this.loadTime = 0;
  this.reqStartTime = 0;
  this.reqEndTime = 0;
  this.reqTestTime = 0;
  this.startExecTime = 0;
  this.endTime = 0;
  this.webdriver = require('webdriver-sync');
  this.By = this.webdriver.By
  this._ = require('lodash');
  this.driver = 0;
  this.assert = require("chai").assert
  this.testIndex = 1;
  this.screenshotPath = ""

  this.fs = require("fs-extra")
  // console.log(process.cwd()+"/../node_modules/webdriver-sync/src/interfaces/OutputType.js");
  // this.OutputType = require(process.cwd()+"../node_modules/webdriver-sync/src/interfaces/OutputType.js");

  var path = require('path');
  this.OutputType = require(
    path.resolve(__dirname, '..', '..', 'webdriver-sync/src', 'interfaces', 'OutputType')
  );

  // console.log(this.config);
}

Test.prototype.showConfig = function () {
  console.log(this.config);
};

Test.prototype.start = function (data) {
  this.screenshotPath = process.cwd()+"/"+this.config.screenshotPath+'/'+this.config.browser+"/"+this.testIndex+"/"
  this.fs.ensureDirSync(this.screenshotPath);
  this.result = data;
  this.initiateDriver();

  if(this.config.screenshot){
    // Take screenshot when browser is ready
    var file = this.driver.getScreenshotAs(this.OutputType.BASE64);
    this.fs.writeFileSync(this.screenshotPath+this.config.browser+"-init"+this.testIndex+".jpg",file,'base64')
  }
  this.openBrowser()
};

Test.prototype.initiateDriver = function () {
  console.log("Initiate Test #"+this.testIndex+" for "+this.config.browser+" : ");
  // console.log(this.config.browser);
  if (this.config.browser == "firefox") {
    // console.log("firefox")
    var FirefoxDriver = this.webdriver.FirefoxDriver;
    this.driver = new FirefoxDriver();
  }else if (this.config.browser == "safari") {
    // console.log("safari")
    var SafariDriver = this.webdriver.SafariDriver;
    this.driver = new SafariDriver();
  }else if (this.config.browser == "chrome") {
    // console.log("chrome");
    var ChromeDriver = this.webdriver.ChromeDriver;
    this.driver = new ChromeDriver();
  }else if (this.config.browser == "ie") {
    console.log("Internet Explorer");
    var IEDriver = this.webdriver.InternetExplorerDriver;
    this.driver = new IEDriver();
  }else if (this.config.browser == "phantomjs") {
    // console.log("Phantom JS");
    var PhantomJSDriver = this.webdriver.PhantomJSDriver;
    this.driver = new PhantomJSDriver();
  }else{
    throw new Error(this.config.browser+" is not browser or not supported yet")
  }
};

Test.prototype.startRequiredTest = function (path , obj, data) {
  obj.reqStartTime = new Date();
  console.log("- Required test started at :"+obj.reqStartTime);
  try{
    require(path)({
      'coba' : obj,
      'data' : data
    });
    obj.reqEndTime = new Date();
  }catch(e){
    // console.log(e.message)
    obj.reqEndTime = obj.reqStartTime;
  }
  console.log("- Required test ended at :"+obj.reqEndTime+"\n");
  // console.log(obj.reqEndTime);
};

Test.prototype.openBrowser = function () {
  this.startTime = new Date()
  console.log("- Load Browser started at :"+this.startTime);
  this.driver.get(this.config.startingUrl)
  this.loadTime = new Date();
  console.log("- Load Browser ended at :"+this.loadTime+"\n");

  if(this.config.screenshot){
    // Take screenshot after loaded
    var file = this.driver.getScreenshotAs(this.OutputType.BASE64);
    this.fs.writeFileSync(this.screenshotPath+this.config.browser+"-load"+this.testIndex+".jpg",file,'base64')
  }
};

Test.prototype.fillInputForm = function (inputs, driver) {
  // console.log(driver)
  var tempBy = this.By
  this._.each(inputs,function(input,index){
    driver.findElement(tempBy.cssSelector(index)).sendKeys(input);
    // console.log("input "+index);
    // webdriver.wait(function() {
    //   return driver.findElements(webdriver.By.cssSelector('.thumbnail').length > 0;
    // }, { timeout: 1000, period: 100 });
  })
};


Test.prototype.startExecution = function (path , obj, data) {
  obj.startExecTime = new Date();
  console.log("- Test Execution started at :"+obj.startExecTime);
  require(path)({
    'coba' : obj,
    'data' : data
  });

  if(this.config.screenshot){
    //Take screenshot after execution
    var file = this.driver.getScreenshotAs(this.OutputType.BASE64);
    this.fs.writeFileSync(this.screenshotPath+this.config.browser+"-exec"+this.testIndex+".jpg",file,'base64')
  }
  obj.stop(new Date())
};


Test.prototype.stop = function (date) {
  this.endTime = date;
  console.log("- Test Execution ended at :"+date+"\n");

  this.result.load = ( this.loadTime - this.startTime ) / 1000;
  this.result.reqTestTime = ( this.reqStartTime - this.reqEndTime ) / 1000;
  this.result.execution = ( this.endTime - this.startExecTime ) / 1000;
  this.result.reqStartTime = this.reqStartTime;
  this.result.reqEndTime = this.reqEndTime;
  this.result.startTime = this.startTime;
  this.result.loadedTime = this.loadTime;
  this.result.endTime = this.endTime;
  this.result.startExecTime = this.startExecTime;
  this.log.push(this.result);
  // console.log("Load time = "+(this.loadTime - this.startTime ) / 1000);
  // console.log("Execution time = "+(this.endTime - this.loadTime ) / 1000);

  this.driver.quit();
};

Test.prototype.setAssertionResultValue = function (value) {
  this.result.assertion = value;
};

Test.prototype.isDeeplyEqual = function (actual,expected) {
  try {
    this.assert.deepEqual(actual,expected)
  } catch (e) {
    this.setAssertionResultValue(false)
    return this.result;
  }
  this.setAssertionResultValue(true)
  return this.result;
};



module.exports = Test
